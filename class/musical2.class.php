...
class musical2 extends SeedObject
{
	/**
	 * Draft status
	 */
	...
	public $table_element = 'musical2';

	public $element = 'musical2';
...
//		global $user;
	...
	private function getNextNumero()
	{
		...
		$numero = get_next_value($db, $mask, 'musical2', 'ref');
...
//		global $user;
	...
	public function getNomUrl($withpicto=0, $get_params='')
	{
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showmusical2") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;
        
        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/musical2/card.php', 1).'?id='.$this->id. $get_params .$linkclose;
	...
	public static function LibStatut($status, $mode)
	{
		...
		if ($status==self::STATUS_DRAFT) { $statustrans='statut0'; $keytrans='musical2StatusDraft'; $shortkeytrans='Draft'; }
		if ($status==self::STATUS_VALIDATED) { $statustrans='statut1'; $keytrans='musical2StatusValidated'; $shortkeytrans='Validate'; }
		if ($status==self::STATUS_REFUSED) { $statustrans='statut5'; $keytrans='musical2StatusRefused'; $shortkeytrans='Refused'; }
		if ($status==self::STATUS_ACCEPTED) { $statustrans='statut6'; $keytrans='musical2StatusAccepted'; $shortkeytrans='Accepted'; }
...
/*
class musical2Det extends TObjetStd
{
	public $table_element = 'musical2det';

	public $element = 'musical2det';
